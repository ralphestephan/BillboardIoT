...
const char* mqtt_server = "13.61.39.237";  // Replace with your broker IP
...
int scanCounter = 0;                      // Counter to track the number of scans
int deviceCounter = 1;                    // Counter for cycling device IDs
...
// Update device_id every 30 scans
void updateDeviceId() {
  if (scanCounter >= 30) {
    scanCounter = 0; // Reset scan counter
    deviceCounter++;
    if (deviceCounter > 20) {
      deviceCounter = 1; // Cycle back to 1 after 20
    }
    device_id = "dv-" + String(deviceCounter).padStart(3, '0');
    Serial.print("Updated device_id: ");
...
void processCurrentCycle() {
  ...
  if (validScanFound && currentResult != previousResult) {
    ...
    scanCounter++; // Increment scan counter after a successful scan
    updateDeviceId(); // Check if the device ID needs updating
  }
}
